...
function processData(input) {
  var domains = input.match(/https?\:\/\/[A-Za-z0-9\-]*\.+[A-Za-z0-9\.\-]*/g),
      dictionary = {};

  domains = domains.map( function(domain) {
      var d = domain.split('//').pop().toLowerCase();

      var subDomain = d.split('.').shift();
      if (subDomain === 'www' ||  subDomain === 'ww1' || subDomain === 'ww2') {
          d = d.split('.');
          d.shift();
          d = d.join('.');
      }
